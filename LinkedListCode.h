...
public:
  ...
  void repeat_push_front(T value, unsigned int repeat) {
    for (unsigned int i = 0; i < repeat; ++i) {
      push_front(value);
    }
  }

  void repeat_push_back(T value, unsigned int repeat) {
    for (unsigned int i = 0; i < repeat; ++i) {
      push_back(value);
    }
  }

  // if position is higher then the size of the LinkedList, the compiler will
  // give a segmentation fault error
  ...
  void insert(unsigned int position, T value) {
    if (head != NULL) {
      // deos a check to see if user is trying to add an eleent to the last of
      // the list
      if (position == size + 2) {
        push_back(value);
      } else {
  ...
  /*You have to #include <functional> and do this if you want compatibilty with
  lambdas, but if you want to do it without #include <functional> or don't care
  about lambdas then do: void(*func)() for the second parameter*/
  ...
  // if you read the comments for the first foreach and also want to replace the
  // second parameter then do this: void(*func)(T)
...
private:
  ...
  // if push_back, push_front, or insert is called and the size is 0 then this
  // funtion is called
  ...
  T get_value(unsigned int position) {
    Node *tempPtr = head;
  ...
  T get_value_of_other(unsigned int position, Node *otherHead) {
  ...
  T &get_value_ref(unsigned int position) {
    Node *tempPtr = head;
...
// cs stands for class, struct
// you should use this instead is you want to make a list of classes or structs
// the reason why is beacuse it limits the mount of copying
template <typename T> class CsLinkedList {
public:
  // Constructors
  CsLinkedList() = default;
  CsLinkedList(std::initializer_list<T> init) {
    for (T element : init) {
      push_back(element);
    }
  }
  CsLinkedList(const LL::LinkedList<T> &other) {
    clear();
    for (int i = 0; i < other.size; ++i) {
      Node *tempPtr = other.head;
  ...
  bool operator==(const LL::CsLinkedList<T> &other) {
    if (other.size != size)
      return false;
  ...
  static void CopyOver(CsLinkedList<T> &copyTo,
                       const CsLinkedList<T> &copyFrom) {
    int size = copyFrom.size;
    copyTo.clear();
    for (int i = 0; i < size; ++i) {
      copyTo.push_back(get_value_of_other(i, copyFrom.head));
